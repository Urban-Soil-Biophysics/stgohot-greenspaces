...
# Start from LST quintiles LANDSAT ------------------------------------------

landsat_spatraster <- rast(lst_quint_stars_landsat)
names(landsat_spatraster) <- "quintile"

# Now as polygon terra (faster for ggplot) --------------------------------

landsat_terra <- terra::as.polygons(
  landsat_spatraster,
  values = TRUE,
  na.rm = TRUE,
  dissolve = TRUE
...
landsat_terra$quintile <- factor(
  landsat_terra$quintile,
  levels = 1:5,
...
ggmap(background_stgo) +
  ...
  labs(x = NULL, y = NULL, title = "Landsat-LST January 20th 2024")

# Ggplot polygons with categories -----------------------------------------
# First, got my analysis to sf
intercept_analysis_sf <-
  intercept_analysis_landsat %>%
...
labs_order <- intercept_analysis_landsat %>%
