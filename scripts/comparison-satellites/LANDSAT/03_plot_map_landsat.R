...
# Background map ----------------------------------------------------------
bg_stgo <- get_stadiamap(         # ✅ consistent variable name bg_stgo
  bbox    = bbox_stgo,
  zoom    = 12,
...
landsat_poly <- terra::as.polygons(
  ...
  st_make_valid() %>%
  mutate(
    decile = factor(decile_landsat, levels = 1:10, labels = paste0("D", 1:10))
  )
...
landsat_deciles_plot <- ggmap(bg_stgo) +
  ...
  labs(x = NULL, y = NULL, title = "Landsat LST deciles — January 2024")

ggsave(
  "outputs/comparison-satellites/LANDSAT/landsat-deciles.tiff",
  landsat_deciles_plot,
  width = 6, height = 4, dpi = 300
)

# Order decile labels -----------------------------------------------------
labs_order_landsat <- intercept_landsat_summary %>%
