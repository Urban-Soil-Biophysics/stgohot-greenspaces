...
minvu <-
  sf::st_read("data/geo-data/parques_urbanos_minvu.geojson") %>%
  mutate(
    greenspace_type = case_when(
      ...
      str_detect(
        NOMBRE,
        regex(
          "bandejon|bandejón|rotonda|av|av.|canal|estero|autopista",
          ignore_case = TRUE
        )
      ) ~ "Roadside",
      TRUE ~ NA_character_
...
intercept_modis <- imap_dfr(modis_extraction, ~ tibble(
  park_id = .y,
  decile = .x[["value"]],
